...
class ROIViewer(widgets.Container):
    ...
    def refresh(self):
        ...
        lvl_idx = self.image.resolution_levels.index(self.resolution_dropdown.value)
        info_dict = self.image.info[lvl_idx]
        info_str = "\n".join(f"{key}: {value}" for key, value in info_dict.items())
        self.level_info.value = info_str

    def toggle_full_view(self):
        idx = self.image.resolution_levels.index(self.resolution_dropdown.value)
        image_size = self.image.info[idx]['image_size']
        if min(image_size) > 1024:
            return
        roi = self.image.rois[idx]
